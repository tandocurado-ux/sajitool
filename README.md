...
あわせて **platform ごとの1枠あたり件数と推奨上限**（Google 4 件・Yahoo! 60 件。
`NEXT_PUBLIC_GOOGLE_SLOT_MAX` / `NEXT_PUBLIC_YAHOO_SLOT_MAX` で変更可）を表示し、
超えていれば黄色で警告して「推奨まで自動分散する」で時間帯・回転数を自動提案する。
Google を含む登録は「時間帯に自動分散」が既定（推奨）になる。

登録済みのスケジュールが1枠に集中してしまった場合は、顧客詳細の
**スケジュールタブ「時刻を撒き直す（再分散）」** で、顧客 × platform（Google だけ /
Yahoo! だけ / 両方）を選び、1枠あたり N 件以内になるよう時刻を振り直せる。
必ずドライラン（撒き直し前後の各枠の件数）を出してから適用する。各スケジュールの
1日の実行回数（時刻の数）は変えず、runs には触らない。適用後に本日の残り時間へ
移った枠は本日からその時刻に実行される。
...
- 顧客詳細のスケジュール一覧の各行にある **「今すぐ実行」**、または新規登録／まとめて登録の
  **「登録して今すぐ1件テスト実行」**（先頭のキーワード × 地域 × デバイスの1件）
- 押すと「実行待ち…」→「実行中…」→ 結果（ok / blocked / error、取得件数、exit IP、最終 URL）が
  その場に出る（3秒ごとのポーリング）。実行中は同じスケジュールのボタンは無効
- 検索フローと runs への記録は定時実行と同じ（Google は Google 対策込み）。
  定時のキュー・当日実行済みの判定・circuit breaker には関与しない

スキーマを変えないため、キューは DB のテーブルではなく **Supabase Auth のユーザー metadata** に置く。
依頼は本人の `user_metadata.immediate_queue`、進捗と結果は計測エンジン（service_role）だけが書く
`app_metadata.immediate_runs`（直近 20 件）。`scheduler.py` が `IMMEDIATE_POLL_SECONDS`（既定 15 秒）
ごとに確認し、依頼があれば定時のキューより先に1件ずつ実行する。`IMMEDIATE_RUNS_ENABLED=0` で無効化。
