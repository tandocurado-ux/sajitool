...
| 時間帯に自動分散 | 開始〜終了（例 06:00〜22:00）を選ぶと、作成するスケジュールを その範囲の15分枠へ均等に割り振る |

自動分散では、枠数と**互いに素な歩幅**で枠を選ぶ。全枠を均等に使いながら、
同じキーワードの4パターンが隣り合う枠に固まらない。
06:00〜22:00（65枠）に40件なら、同じキーワードは 06:00 / 10:00 / 14:00 / 18:00 に散る。

件数が多い顧客は自動分散を使うこと。全件同じ時刻にすると1つの枠に集中し、
スケジューラのキュー上限で捨てられる。
...
- 実行と実行の間に **60〜180秒のランダム間隔**を空ける（毎時0分の連射を避ける）。
  範囲は `RUN_INTERVAL_MIN_SECONDS` / `RUN_INTERVAL_MAX_SECONDS` で変えられる
...
- キューは **`QUEUE_MAX_ITEMS`（既定 60）件が上限**。超えた分は積まずに捨て、
  警告とアラートを出す（当日中の再実行はしない）
...
1枠にいくつ入るかの目安は **3600 ÷ 平均間隔**。既定（平均120秒）なら30件までが安全圏。
`QUEUE_MAX_ITEMS` を超える分はそもそも積まれないので、件数が多い顧客は
一括登録の「時間帯に自動分散」で時刻をばらしておくこと。
...
2. 作成された Worker の **Environment** に次を設定する（`render.yaml` には値を置かない）
   ...
   | `RUN_INTERVAL_MIN_SECONDS` | 任意 | 既定 `60`。実行間隔の下限 |
   | `RUN_INTERVAL_MAX_SECONDS` | 任意 | 既定 `180`。実行間隔の上限 |
   | `QUEUE_MAX_ITEMS` | 任意 | 既定 `60`。キューに積む上限 |
