...
export function FailureBreakdown({
  ...
  return (
    <span className="text-xs text-neutral-500">
      <span className={counts.blocked > 0 ? "font-medium text-red-600" : undefined}>
        {RUN_STATUS_LABELS.blocked} {counts.blocked}
      </span>
      {" / "}
      <span className={counts.error > 0 ? "font-medium text-amber-700" : undefined}>
...
export function DailyProgressSummary({ progress }: { progress: DailyProgress }) {
  ...
  return (
    <span className="flex flex-col gap-0.5">
      ...
      <span
        className={`text-xs ${delayed ? "font-medium text-amber-700" : "text-neutral-500"}`}
